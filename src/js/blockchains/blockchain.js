...
export class Blockchain extends createjs.Container {
	...
	start() {
		...
		this.minuteListener = window.Stage.on('newminute', this.onNewMinute, this);
	}

	stop() {

		this.mempool.stop();
		if(this.minuteListener) window.Stage.off('newminute', this.minuteListener);
	}
	...
	mineBlock() {
		...
		let block = new Block({blockchain: this, blockheight: this.blockheight});
		let timex = window.Timelines.currentBar.localToLocal(0,0, this).x;
	...
	notarize() {
		...
		let blockchain = window.Blockchains.find(b => b.params.id == dest);
	...
	drawNotarySecurity(block, transactions) {
		...
		notary.map(function(t) {
			...
			if(blockchain.params.id == 'btc') {
				icon = new createjs.Bitmap(window.Queue.getResult('btc_security'));
			}
			...
			let tw2 = createjs.Tween.get(icon, {timeScale: window.TimeScale}).wait(1000).to({alpha: 1, scaleX: scale, scaleY: scale}, 650, createjs.Ease.bounceOut);
